...
class ModelTrainer:
    ...
    def train(
        ...
        train_with_dev: bool = False, # NOTE: We should set this to False. 
        ...
        **kwargs,
        ...
        # preprocess for mix
        # 0. index the training data
        for i,sentence in enumerate(train_data.sentences):
            sentence.add_idx(i)
        assert i==len(train_data)-1,(i,len(train_data))

        # 1. load the knn file        
        if args.use_knn_train_data:
            file_path = os.path.join(args.knn_idx_file)
            I_array = pickle.load(open(file_path, 'rb'))
            sent_id_knn_array = I_array[:,1:args.num_knn_k+1]
            assert sent_id_knn_array.shape[1]==args.num_knn_k
        ...
        try:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    if args.mix_option:
                        mix_batch=utils.gen_knn_mix_batch(batch=batch,
                                                train_dataset=train_data,
                                                sent_id_knn_array=sent_id_knn_array,
                                                knn_mix_ratio=args.knn_mix_ratio,
                                                train_size=args.train_examples)
                        assert len(batch)==len(mix_batch)
                    ...
                    if args.mix_option:
                        mix_batch_steps = [mix_batch]
                        if len(mix_batch) > micro_batch_size:
                            mix_batch_steps = [
                                mix_batch[x : x + micro_batch_size]
                                for x in range(0, len(mix_batch), micro_batch_size)
                            ]

                    # forward and backward for batch
                    if args.mix_option:
                        for batch_step,mix_batch_step in zip(batch_steps,mix_batch_steps):

                            if (args.alpha-0)<1e-6:
                                mix_ratio=1
                            else:
                                if args.beta==-1:
                                    mix_ratio = np.random.beta(args.alpha, args.alpha)
                                else:
                                    mix_ratio = np.random.beta(args.alpha, args.beta)
                                mix_ratio = max(mix_ratio, 1-mix_ratio)
                            if args.mix_layer in [0,1]:
                                mix_layer=args.mix_layer
                            elif args.mix_layer==2:
                                mix_layer = random.choice([0,1])
                            else:
                                raise ValueError('mix_layer must be in [0,1,2]')
                        ...
                        for batch_step in batch_steps:    
                            loss = self.model.forward_loss(batch_step,None)
                            # Backward
                            if use_amp:
                                with amp.scale_loss(loss, optimizer) as scaled_loss:
                                    scaled_loss.backward()
                    ...
                    store_embeddings(batch, embeddings_storage_mode) # NOTE: this is the problem.
                    if args.mix_option:
                        store_embeddings(mix_batch, embeddings_storage_mode) # TODO: do we have to store the mix embeddings?
                ...
                if (
                    ...
                    # test_f1_history.append(final_score)
