...
def pad_tensors_max_len(tensor_list,max_len):
    ml = max_len
    shape = [len(tensor_list), ml] + list(tensor_list[0].shape[1:])
    template = torch.zeros(*shape, dtype=torch.long, device=flair.device)
    lens_ = [x.shape[0] for x in tensor_list]
...
class SequenceTagger(flair.nn.Model):
    ...
    def forward_loss(
        ...
        mix_data_points=None,#:  Union[List[Sentence], Sentence],
        mix_layer=None,#: int,[0,1]
        mix_ratio=None,# float
        sort=True
    ) -> torch.tensor:
        features = self.forward(data_points,mix_data_points,mix_layer,mix_ratio)
        return self._calculate_loss(features, data_points,mix_data_points,mix_ratio)

    def forward(
        self, 
        sentences: List[Sentence],
        mix_sentences=None,#: List[Sentence],#: List[Sentence],
        mix_layer=None,#: int,[0,1]
        mix_ratio=None,# float
        ): 
        ...
        if mix_sentences is not None:
            self.embeddings.embed(mix_sentences)             
            mix_lengths: List[int] = [len(sentence.tokens) for sentence in mix_sentences]
            mix_longest_token_sequence_in_batch: int = max(mix_lengths)
            longest_token_sequence_in_batch = max(longest_token_sequence_in_batch,mix_longest_token_sequence_in_batch)        
            # we need a max_lengths for both lengths and mix_lengths
            max_lengths=[]
            for l,mix_l in zip(lengths,mix_lengths):
                max_lengths.append(max(l,mix_l))
            assert len(max_lengths)==len(lengths)
        ...
        if mix_sentences is not None:
            mix_pre_allocated_zero_tensor = torch.zeros(
                self.embeddings.embedding_length * longest_token_sequence_in_batch,
                dtype=torch.float,
                device=flair.device,
            )
        ...
        if mix_sentences is not None:
            for sentence in mix_sentences:
                ...
                if nb_padding_tokens > 0:
                    t = mix_pre_allocated_zero_tensor[
                        : self.embeddings.embedding_length * nb_padding_tokens
                    ]
                    mix_all_embs.append(t)
        ...
        if mix_sentences is not None:
            mix_sentence_tensor = torch.cat(mix_all_embs).view(
                [
                    len(mix_sentences),
                    longest_token_sequence_in_batch,
                    self.embeddings.embedding_length,
        ...
        if mix_sentences is not None:
            if self.use_dropout > 0.0:
                mix_sentence_tensor = self.dropout(mix_sentence_tensor)
            if self.use_word_dropout > 0.0:
                mix_sentence_tensor = self.word_dropout(mix_sentence_tensor)
            if self.use_locked_dropout > 0.0:
                mix_sentence_tensor = self.locked_dropout(mix_sentence_tensor)
            if self.relearn_embeddings:
                mix_sentence_tensor = self.embedding2nn(mix_sentence_tensor)
            if mix_layer==0:
                sentence_tensor = mix_ratio*sentence_tensor+(1-mix_ratio)*mix_sentence_tensor


        if self.use_rnn:
            if mix_sentences is not None and mix_layer==0:

                packed = torch.nn.utils.rnn.pack_padded_sequence(
                    sentence_tensor, max_lengths, enforce_sorted=False, batch_first=True
                )
            ...
            if mix_sentences is not None and mix_layer==1:
                mix_packed = torch.nn.utils.rnn.pack_padded_sequence(
                    mix_sentence_tensor, mix_lengths, enforce_sorted=False, batch_first=True
                )

                # if initial hidden state is trainable, use this state
                if self.train_initial_hidden_state:
                    mix_initial_hidden_state = [
                        self.lstm_init_h.unsqueeze(1).repeat(1, len(mix_sentences), 1),
                        self.lstm_init_c.unsqueeze(1).repeat(1, len(mix_sentences), 1),
                    ]
                    mix_rnn_output, mix_hidden = self.rnn(mix_packed, mix_initial_hidden_state)
                else:
                    mix_rnn_output, mix_hidden = self.rnn(mix_packed)

                mix_sentence_tensor, mix_output_lengths = torch.nn.utils.rnn.pad_packed_sequence(
                    mix_rnn_output, batch_first=True
                )
                ...
                # mix here
                # FIXME: pad one tensor, here we use zero-padding, not sure if this is correct.
                if sentence_tensor.shape[1]>mix_sentence_tensor.shape[1]:
                    # we need to pad mix_sentence_tensor to 
                    padding_zero_tensor=torch.zeros(
                        sentence_tensor.shape,
                        dtype=torch.float,
                        device=flair.device,
                    )
                    padding_zero_tensor[:,0:mix_sentence_tensor.shape[1],:]=mix_sentence_tensor
                    mix_sentence_tensor = padding_zero_tensor
                elif sentence_tensor.shape[1]<mix_sentence_tensor.shape[1]:
                    # we need to pad sentence_tensor to 
                    padding_zero_tensor=torch.zeros(
                        mix_sentence_tensor.shape,
                        dtype=torch.float,
                        device=flair.device,
                    )
                    padding_zero_tensor[:,0:sentence_tensor.shape[1],:]=sentence_tensor
                    sentence_tensor = padding_zero_tensor
                sentence_tensor = mix_ratio*sentence_tensor+(1-mix_ratio)*mix_sentence_tensor
    ...
    def _calculate_loss(
        ...
        sentences: List[Sentence],
        mix_sentences=None,#: List[Sentence],
        mix_ratio=None, # float
    ) -> float:
        ...
        if mix_sentences is not None:
            ...
            mix_tag_list: List = []
            for s_id, sentence in enumerate(mix_sentences):
                # get the tags in this sentence
                mix_tag_idx: List[int] = [
                    self.tag_dictionary.get_idx_for_item(token.get_tag(self.tag_type).value)
                    for token in sentence
                ]
                # add tags as tensor
                mix_tag = torch.tensor(mix_tag_idx, device=flair.device)
                mix_tag_list.append(mix_tag)
                            

        if self.use_crf:
            if mix_sentences is not None:
                max_len = max( max(lengths),max(mix_lengths) )
            else:
                max_len = max(lengths)

            # pad tags if using batch-CRF decoder
            # pad_tensors_max_len(tensor_list,max_len)
            tags, _ = pad_tensors_max_len(tag_list,max_len)
            ...
            if mix_sentences is not None:
                ...
                for l,mix_l in zip(lengths,mix_lengths):
                    max_lengths.append(max(l,mix_l))
                forward_score = self._forward_alg(features, max_lengths)
                gold_score = self._score_sentence(features, tags, max_lengths) 
            else:
            ...
            if mix_sentences is not None:
                mix_tags, _ = pad_tensors_max_len(mix_tag_list,max_len)        
                mix_gold_score = self._score_sentence(features, mix_tags, max_lengths)
                
                gold_score = mix_ratio*gold_score+(1-mix_ratio)*mix_gold_score
            ...
            if mix_sentences is not None:
                mix_score = 0
                for sentence_feats, sentence_tags, sentence_length in zip(
                    features, mix_tag_list, mix_lengths
                ):
                    sentence_feats = sentence_feats[:sentence_length]
                    mix_score += torch.nn.functional.cross_entropy(
                        sentence_feats, sentence_tags, weight=self.loss_weights
                    )
                mix_score /= len(features)
                score = mix_ratio*score+(1-mix_ratio)*mix_score
